...
const AnimalTab = () => {
  ...
  const onSubmit = async (data: z.infer<typeof CreateAnimalSchema>) => {
    try {
      ...
      if (err instanceof ApiError) {
        if (err.detail === "No entiendo que onda con tus credenciales") { // se vencio el token xD
          logout();
  ...
  return (
    <>
      <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight first:mt-0">Publicar nuevo animal en adopción</h2>
